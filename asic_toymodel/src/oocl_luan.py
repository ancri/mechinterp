...
class Tokens:
    ...
    equal: int = 0  # 120
    # diff from 2nv
    reliable_def: int = 1  # 241
    unreliable_def: int = 2  # 242
    padding: int = 3  # 243
...
def create_data(int_by_set, prop_val=0.1, num_questions=6):
    ...
    # adding a bit of X1 into X2 training sets
    size_x1 = train_sets['X1'].size(0)
    size_x2 = train_sets['X2'].size(0)
    k = size_x2
    perm = torch.randperm(size_x1)
    idx = perm[:k]
    print(f"Before adding some X1, the size of X2 training set is {size_x2}")
    train_sets['X2'] = torch.cat((train_sets['X2'], train_sets['X1'][idx]), dim=0)
    print(f"After adding some X1, the size of X2 training set is {train_sets['X2'].size(0)}")
...
def check_save_model(model, run_name, args, cur_step):

    if cur_step in args.save_steps:
        model_name = f"oocl_{run_name}_{DataParams.mod}_step_{cur_step}.pt"
        model_path = os.path.join(args.model_path, model_name)
        torch.save(model.state_dict(), model_path)


def train_w_orig(model, train_sets, test_sets, orig_args, train_params, run_name, args):
    ...
    for epoch in range(train_params.num_epochs_X1):
        ...
        check_save_model(model, run_name, args, epoch)
    ...
    for epoch in range(train_params.num_epochs_X2):
        ...
        check_save_model(model, run_name, args, train_params.num_epochs_X1 + epoch)
...
if __name__ == '__main__':
    ...
    for i_run in range(args.n_runs):
        ...
        dir_assets = Path("models/transformers")
        fpath_numbers = dir_assets / f"numbers_{run_name}.pkl"
        pickle.dump(int_by_set, open(fpath_numbers, "wb"))
        ...
        train_w_orig(new_model, train_sets, test_sets, orig_args, train_params, run_name, args)
